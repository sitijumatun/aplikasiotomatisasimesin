...
<head>
  ...
  <title>Halaman Login Stanley</title>
...
<div class="login-box">
  ...
  <div class="login-box-body">
    <h4 class="login-box-msg">Silakan login</h4>
	...
	$sqlnama = "SELECT * FROM apl_user WHERE username='$username2' && password='$password2'";
			...
			if(mysqli_num_rows($resultnama)>0){
					while($row = mysqli_fetch_assoc($resultnama)){
							...
							$SESSION['role']=$role;
							if($SESSION['role']==1){
								echo '<script language="javascript">alert("Anda berhasil login sebagai Operator !"); document.location="apl_operatorhome.php";</script>';
								
							}
							if($SESSION['role']==2){
								echo '<script language="javascript">alert("Anda berhasil login sebagai Admin !"); document.location="apl_adminlkh.php";</script>';
							}								
					}
			}
			else {
					
					echo '<script language="javascript">alert("Maaf Username dan Password tidak terdaftar, Hubungi Admin !"); document.location="apl_login.php";</script>';
